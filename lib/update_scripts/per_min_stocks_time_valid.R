...
### This Script Needs stocks_list.csv file in Data folder
...
TwsPerminStocksValidateForTime <- function(
...
for(i in 1 :no)
{
  ...
  { 
    ...
    tab<-(dbGetQuery(connection.TWS_PERMIN_stocks,query)) ## implements the query
    cat("TWS_PERMIN_stocks" ,timestamp(),"TwsPerminStocksValidateForTime",paste("Read", stock ,sep=" "),"\n",file = connection.log, sep = ",")
    dates<-(tab[1])
    ...
    cat("VALIDATION_TWS_PERMIN_stocks" ,timestamp(),"TwsPerminStocksValidateForTime",paste(str, tablename ,sep=" "),"\n",file = connection.log, sep = ",")
    ifelse(dbExistsTable(connection.VALIDATION_TWS_PERMIN_stocks, tablename),dbWriteTable(connection.VALIDATION_TWS_PERMIN_stocks, name =tablename, value=data, append = T),dbWriteTable(connection.VALIDATION_TWS_PERMIN_stocks, name = tablename, value=data))
  } 
  ,silent=TRUE)
}
close(connection.log)
dbDisconnect(connection.TWS_PERMIN_stocks)
dbDisconnect(connection.VALIDATION_TWS_PERMIN_stocks)
}