...
NseOhlcFuturesValidateForTime <- function(
user.name = "intern@Ophelia"
  ...
  connection.VALIDATION_NSE_OHLC_futures <- CreateConnection(user.name , machines ,"VALIDATION_NSE_OHLC_futures")
  ...
  for(i in 1 :no)
  {
    ...
    tab<-(dbGetQuery(connection.NSE_OHLC_futures,query)) ## implements the query
    cat("NSE_OHLC_futures" ,timestamp(),"NseOhlcFuturesValidateForTime",paste("Read", stock ,sep=" "),"\n",file = connection.log, sep = ",")
    print(stock)
    dates<-(tab[1])
    ...
    ifelse(dbExistsTable(connection.VALIDATION_NSE_OHLC_futures, tablename),dbWriteTable(connection.VALIDATION_NSE_OHLC_futures, name =tablename, value=data, append = T),dbWriteTable(connection.VALIDATION_NSE_OHLC_futures, name = tablename, value=data))
    ...
    if (flag > 0)
    {
     ...
     ,
     "Added" , "Not Added")
     cat("VALIDATION_NSE_OHLC_futures" ,timestamp(),"NseOhlcFuturesValidateForTime",paste(str, tablename ,sep=" "),"\n",file = connection.log, sep = ",")
     ifelse(dbExistsTable(connection.VALIDATION_NSE_OHLC_futures, tablename),dbWriteTable(connection.VALIDATION_NSE_OHLC_futures, name =tablename, value=data, append = T),dbWriteTable(connection.VALIDATION_NSE_OHLC_futures, name = tablename, value=data))
     print(nrow(data))
    }
...
dbDisconnect(connection.NSE_OHLC_futures)
dbDisconnect(connection.VALIDATION_NSE_OHLC_futures)
}