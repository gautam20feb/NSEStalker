...
### It produces for each stock in the database TWS_OHLC_stocks a table having the information about breaks
...
TwsOhlcStocksValidateForTime <- function(
user.name = "intern@Ophelia"
  ...
  for(i in 1 :no)
  {
    ...
    tab<-(dbGetQuery(connection.TWS_OHLC_stocks,query)) ## implements the query
    cat("TWS_OHLC_stocks" ,timestamp(),"TwsOhlcStocksValidateForTime",paste("Read", stock ,sep=" "),"\n",file = connection.log, sep = ",")
    print(stock)
    dates<-(tab[1])
    ...
    ifelse(dbExistsTable(connection.VALIDATION_TWS_OHLC_stocks, tablename),dbWriteTable(connection.VALIDATION_TWS_OHLC_stocks, name =tablename, value=data, append = T),dbWriteTable(connection.VALIDATION_TWS_OHLC_stocks, name = tablename, value=data))
    ...
    if (flag > 0)
    {
     ...
     ,
     "Added" , "Not Added")
     cat("VALIDATION_TWS_OHLC_stocks" ,timestamp(),"TwsOhlcStocksValidateForTime",paste(str, tablename ,sep=" "),"\n",file = connection.log, sep = ",")
     ifelse(dbExistsTable(connection.VALIDATION_TWS_OHLC_stocks, tablename),dbWriteTable(connection.VALIDATION_TWS_OHLC_stocks, name =tablename, value=data, append = T),dbWriteTable(connection.VALIDATION_TWS_OHLC_stocks, name = tablename, value=data))
     print(nrow(data))
    }
...
dbDisconnect(connection.VALIDATION_TWS_OHLC_stocks)
}